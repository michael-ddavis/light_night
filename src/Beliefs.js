...
const Beliefs = () => {
  return (
    <div className="container flex flex-col mx-auto px-6 md:px-14 lg:px-20">
        <h1 className="mx-auto text-5xl font-semibold py-8 md:py-4">Our Beliefs</h1>
      {beliefs.map((belief, index) => (
        <div className="py-6" key={index}>
          <h3 className="text-3xl font-medium pb-3">
            {belief.title}
          </h3>
          <p className="text-justify prose prose-indigo prose-lg">
            {belief.description}
          </p>
          <div className="flex flex-wrap gap-4 pt-3 lg:pt-5">
            {belief.references.map((scripture, index) => (
              <div key={index} className="rounded-full py-1 px-3 text-xs font-medium md:font-semibold bg-gray-200 text-color-black flex-shrink-0 flex-none">
                {scripture}
              </div>
            ))}
          </div>
